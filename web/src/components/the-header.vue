    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <button v-show="loginMember.id" v-on:click="logout()"
                  class="btn btn-outline-light my-2 my-sm-0">退出登录</button>
          <!--互斥显示很常见，比如收缩展开，收费免费，显示一个就不显示另一个。-->
          <button v-show="!loginMember.id" v-on:click="openLoginModal()"
                  class="btn btn-outline-light my-2 my-sm-0">登录/注册</button>
        </div>
      </div>
    </nav>
...
export default {
  ...
  methods:{
    ...
    logout(){
      let _this = this;
      _this.$ajax.get(process.env.VUE_APP_SERVER + '/business/web/member/logout/'
          + _this.loginMember.token).then((response)=>{
