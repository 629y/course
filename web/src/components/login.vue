              ...
              <div class="input-group">
                ...
                <div class="input-group-append">
                  <button v-on:click="sendSmsForForget()"
                          class="btn btn-outline-secondary" id="forget-send-code-btn">发送验证码
                  </button>
                </div>
              </div>
  ...
  export default {
    ...
    methods:{
      ...
      login(){
        ...
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/member/login',_this.member)
            .then((response)=>{
          let resp = response.data;
          if (resp.success){
      ...
      loadImageCode:function (){
        ...
        $('#image-code').attr('src',process.env.VUE_APP_SERVER +
            '/business/web/kaptcha/image-code/' + _this.imageCodeToken);
      },
      /**
       * 发送注册短信
       */
      sendSmsForRegister(){
        ...
        _this.$ajax.get(process.env.VUE_APP_SERVER
            + '/business/web/member/is-mobile-exist/' + _this.memberRegister.mobile).then((res)=>{
      ...
      sendSmsForForget(){
        ...
        _this.$ajax.get(process.env.VUE_APP_SERVER
            + '/business/web/member/is-mobile-exist/' + _this.memberForget.mobile).then((res)=>{
          let response = res.data;
          if (response.success){
      ...
      resetPassword(){
        ...
        // 调服务端密码重置接口
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/web/member/reset-password',
            _this.memberForget).then((res)=>{
          let response = res.data;
          if (response.success) {
      ...
      onRegisterConfirmPasswordBlur(){
        ...
        return _this.registerConfirmPasswordValidate = (confirmPassword === _this.memberRegister.confirm);
      },
      //------------忘记密码框校验----------------
      onForgetMobileBlur(){
        let _this = this;
      ...
      onForgetConfirmPasswordBlur(){
        ...
        return _this.forgetConfirmPasswordValidate = (forgetPassword === _this.memberForget.confirm);
      },
    }
  }
</script>
